...
public class BusinessScopeFragment extends BaseFragment {
     ...
     * 获取图片
     */
    private void queryPepperBusiness() {
        Observable<Result> observable = NetClient.getInstance(NetClient.getBaseUrl(), false).getZbsApi().queryPepperBusiness();
        observable.subscribeOn(Schedulers.io()).observeOn(AndroidSchedulers.mainThread()).subscribe(new NetworkObserver<Result>(mContext) {
            ...
            public void onNext(Result result) {
                if (result.getCode() == ErrorCode.SUCCESS) {
                    ...
                    SPHelper.save("business", GsonUtils.convertJSON(dataBeans));
    ...
    private void loadLastPictures() {
        List<DataBean> dataBeans = GsonUtils.parseJSONList(SPHelper.getString("business", GsonUtils.convertJSON(new ArrayList<>())), DataBean.class);
        banner.setAdapter(new ImageAdapter(mContext, dataBeans))
                .addPageTransformer(new ScaleInTransformer())
                .setIndicator(new NumIndicator(mContext))
                .setIndicatorGravity(IndicatorConfig.Direction.RIGHT)
